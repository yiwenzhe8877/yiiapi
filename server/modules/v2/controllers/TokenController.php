...
class TokenController extends Controller
{
    ...
    public  $map=[
        ...
        '获得全部组信息'=>['service'=>'group.update','id'=>"1",'status'=>"1",'del'=>"1"],
        '组更新'=>['service'=>'group.getall'],
        '管理组的添加'=>['service'=>'group.add','group_name'=>'宇宙第一'],
        //管理员
        'user.login'=>['service'=>'user.login','username'=>'admin','password'=>"123123"],
        'user.delete'=>['service'=>'user.delete','id'=>'1'],
        'user.getadmin'=>['service'=>'user.getadmin'],
        'user.getlist'=>['service'=>'user.getlist','pageNum'=>1],
        'user.update'=>['service'=>'user.update','id'=>'1','nickname'=>"123123"],
    ...
    public function actionTest()
    {
        ...
        $map=$this->map;
        foreach ($map as $k=>$v){
            if($this->post($url,$v)=='ok'){
                $z=$k.'-----:'.$v['service'].'-----'.$this->post($url,$v);
